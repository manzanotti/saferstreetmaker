...
export const initTabs = (): void => {
  const tabLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>('[data-tab-toggle]'));
  ...
  const activate = (link: HTMLAnchorElement): void => {
    ...
    tabLinks.forEach((other) => {
      ...
      if (panelSelector) {
        document.querySelector(panelSelector)?.removeAttribute('data-tab-active');
      }
    });
