...
interface PolygonHistoryFeature {
...
function buildPolygonPointChanges(
    beforeCoordinates: number[][][],
    afterCoordinates: number[][][]
): PolygonPointChange[] | null {
...
export function getPolygonMutationPayload(
    beforeFeature: PolygonHistoryFeature | null | undefined,
    afterFeature: PolygonHistoryFeature | null | undefined,
    defaultColor: string
) {
    ...
    return {
        ...
        beforeColor: beforeFeature?.properties?.color ?? defaultColor,
        afterColor: afterFeature?.properties?.color ?? defaultColor
    };
}
